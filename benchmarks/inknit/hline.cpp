...
APPLY(arith) {
	for (int i = 0; i < ITERATIONS; ++i) {
		INKNIT_INTERNAL_FUNC(draw_hline_arith, CURRENT_GROUP)(
			static_cast<uint32_t *>(image_.data()), image_.stride(), x1_, x2_, y_, COLOR_WHITE
		);
	}
}
...
APPLY(lut) {
	for (int i = 0; i < ITERATIONS; ++i) {
		INKNIT_INTERNAL_FUNC(draw_hline_lut, CURRENT_GROUP)(
			static_cast<uint32_t *>(image_.data()), image_.stride(), x1_, x2_, y_, COLOR_WHITE
		);
	}
}
