...
void subtest_fill_rect_coord(
	Image& image, point_t pt1, point_t pt2, color_t color = colors::white
) noexcept {
	image.clear(colors::black);
	image.fill_rect_coord(pt1, pt2, color);
	image.test(bind_is_pixel_on_rect_with_point(pt1, pt2));
}
...
#define SUBCASE_INVOKE(_IX, _IY, _IW, _IH, _MSG)                            \
	do {                                                                    \
		SUBCASE(_MSG) {                                                     \
			shared::subtest_fill_rect_coord(image, {_IX, _IY}, {_IW, _IH}); \
		}                                                                   \
	} while (false)
...
#undef SUBCASE_INVOKE
	...
	SUBCASE("single-column") {
		image.reset(1, 8);
		shared::subtest_fill_rect_coord(image, {0, 1}, {0, 3});
	}

	SUBCASE("single-row") {
		image.reset(8, 1);
		shared::subtest_fill_rect_coord(image, {3, 0}, {5, 0});
	}
}
