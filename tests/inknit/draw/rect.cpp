...
void subtest_draw_rect(
	Image& image, point_t pt, size_t sz, color_t color = colors::white
) noexcept {
	image.clear(colors::black);
	image.draw_rect(pt, sz, color);
	image.test(bind_is_pixel_on_rect_edge(pt, sz));
}
...
#define SUBCASE_INVOKE(_IX, _IY, _IW, _IH, _MSG)                      \
	do {                                                              \
		SUBCASE(_MSG) {                                               \
			shared::subtest_draw_rect(image, {_IX, _IY}, {_IW, _IH}); \
		}                                                             \
	} while (false)
...
#undef SUBCASE_INVOKE
	...
	SUBCASE("single-column") {
		image.reset(1, 8);
		shared::subtest_draw_rect(image, {0, 1}, {1, 3});
	}

	SUBCASE("single-row") {
		image.reset(8, 1);
		shared::subtest_draw_rect(image, {3, 0}, {3, 1});
	}
}
